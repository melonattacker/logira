...
static __always_inline int read_sockaddr(const struct sockaddr *addr, __u64 addrlen, struct addr_info *out) {
    if (!addr || !out) {
        return -1;
    }
...
int trace_enter_connect(struct trace_event_raw_sys_enter *ctx) {
    ...
    state.op = OP_CONNECT;
    read_sockaddr((const struct sockaddr *)ctx->args[1], ctx->args[2], &state.addr);
...
int trace_enter_sendto(struct trace_event_raw_sys_enter *ctx) {
    ...
    state.fd = (__s32)ctx->args[0];
    state.op = OP_SEND;
    read_sockaddr((const struct sockaddr *)ctx->args[4], ctx->args[5], &state.addr);
